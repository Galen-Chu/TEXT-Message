...
> 隱私說明:本專案**沒有後端**。Gmail 功能採瀏覽器端 Google OAuth(**唯讀**範圍):access token 僅存記憶體、中斷連線即撤銷,郵件解析與分類都在使用者瀏覽器內完成,不會上傳任何伺服器;AI 功能採 BYOK——Gemini API key 僅存各自瀏覽器,由瀏覽器直接呼叫 Google。未連線時的郵件與社群發文歷史仍為示範假資料(`src/data/mockData.ts`);自行新增的範本與排程存於**各自瀏覽器的 localStorage**(重新整理不消失,清除瀏覽器資料即重置)。詳見 [`docs/SETUP.md`](docs/SETUP.md) §9。
...
| 草稿撰寫 | 來源參考(郵件 / 社群歷史)、AI 語氣改寫與自訂指令、AI 郵件摘要、文管庫插入、社群媒體挑選、多平台預覽與字數上限檢查 |
