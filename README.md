...
個人自媒體經營者用的每日社群發文工具。核心流程:**Gmail 內容 / 文庫常用內容 / 過去社群貼文 → 貼文草稿 → 跨平台排程**。
...
| 頁面 | 說明 |
| --- | --- |
| 文管 Dashboard | 本週統計卡、近期排程、來自 Gmail 的靈感,一鍵轉草稿;發文趨勢精簡版(近 30 天平台計數與連續天數,累積 5 筆真實記錄後出現) |
| 郵件匣 Gmail | **連接真實 Gmail(瀏覽器端唯讀 OAuth)**:近 7 天郵件、搜尋 / 標籤篩選(規則式分類)、**Gmail 使用者標籤第二篩選維度**、「自動分類」一鍵開 Gmail 建立原生篩選器(唯讀輔助)、建議可發文標示、轉為草稿、載入更多與前景自動刷新;未連線時為示範模式 |
| 雲端列 Drive | **連接 Google Drive(唯讀)**:搜尋自己儲存的 Docs/純文字文章、純文字預覽、引用到編發器作為素材([`docs/DRIVE-PLAN.md`](docs/DRIVE-PLAN.md);二期規劃:AI 參照行文風格、存為範本);未連線時為示範文檔 |
| 自媒體 Social | 各平台已發佈貼文歷史,平台篩選,可作為草稿內容來源;在排程頁「標記已發佈」後即為真實記錄(無記錄時為示範資料);單筆記錄可「存為範本」進文庫;**發文趨勢**(近 30/90 天平台計數、時段分佈、連續天數——僅計真實記錄) |
| 編發器 Text | 來源參考(郵件 / 社群歷史)、AI 語氣改寫與自訂指令、AI 郵件摘要、**AI 產生平台版本**(依勾選平台生成各版,可編輯後附加或存為範本)與**建議標籤**、文庫插入(含 `{{變數}}` 填值)、社群媒體挑選、多平台預覽與字數上限檢查;**YouTube 影片/Shorts 直接上傳**(草稿作為標題與說明,支援原生預約發佈);草稿自動保存於瀏覽器,可捨棄重來 |
| 定排程 Task | 週曆檢視、排程新增 / 編輯 / 刪除;狀態流轉(草稿 / 已確認 / 逾期 / 已發佈)與逾期提示;發佈輔助:一鍵複製 + 開啟平台(Threads 可預填文字);設定後端時另有 **Threads 雲端佇列**(到點自動發佈,可取消) |
| 文庫 Library | 三分頁:**訊息管理**(通用文字片段)與**文案管理**(完整貼文範本),分類 / 搜尋 / 排序(最常用 / 最近使用)/ 套用 / 複製(含變數填值)/ 新增 / 編輯 / 刪除,卡片顯示使用統計,**平台變體**:同一範本可存各平台專屬版本;**草稿管理**(IA Phase 2):編輯器「儲存草稿」的保存成果,依更新時間排序,可開啟至編輯器 / 複製 / 刪除 |
...
- **Phase 1(2026-09-04)**:六頁籤雙語更名與順序 v3——**文管 Dashboard → 郵件匣 Gmail → 自媒體 Social → 編發器 Text → 定排程 Task → 文庫 Library**(編發器與定排程對調);品牌字串與 `<title>` 改為 **TEXT-Message**;Dashboard 移除「從 Gmail 建立草稿」按鈕(決策記錄見 [`docs/IA-PLAN.md`](docs/IA-PLAN.md))
...
- **Phase 4(2026-09-07)**:排程對應三大類——排程列類別徽章與篩選(全部/草稿/文案/訊息),新增/編輯可選類型,編輯器加入排程自動帶入當前類型
- **雲端列 Drive（2026-09-08，一、二期同日完成）**：第七頁籤，Google Drive 唯讀串接——搜尋、純文字預覽、引用到編發器；二期：**AI 風格參照**（至多 3 篇樣本，生成模仿你的行文；樣本僅存中繼資料，內容不落地）與**存為範本**寫入文庫三大類。見 [`docs/DRIVE-PLAN.md`](docs/DRIVE-PLAN.md)
- **UX 優化(2026-09-07 晚,D12)**:文庫「套用到草稿」改「套用編發器」、「代發」用語全面改「發佈」、「編輯器 Text」更名**編發器 Text**、草稿管理內建預設電子郵件範本 3 組、OAuth 授權分頁完成後自動返回原分頁
- **Phase 5 一期(2026-09-07)**:Gmail 互動通知分類——郵件匣新增「互動通知」類別(平台留言/提及/追蹤通知信),自媒體頁「📬 最新互動」卡([`docs/NOTIFY-PLAN.md`](docs/NOTIFY-PLAN.md):二期 Threads 輪詢等方案待拍板)
...
- **文庫後端儲存**(訊息管理 + 文案管理):目前以瀏覽器 localStorage 暫存(排程亦同),無跨裝置同步
...
**前端為主、後端輔助(可選)**。已完成:排程管理與發佈輔助(階段一)、**YouTube 影片/Shorts 上傳**(階段二,瀏覽器端 Google OAuth、零後端)、**文管庫(現文庫)功能深化四期**(範本填值、平台變體、發文趨勢、AI 平台版本與標籤建議——[`docs/LIBRARY-PLAN.md`](docs/LIBRARY-PLAN.md))、**Threads 平台發佈前端串接**(階段三——設定後端網址(`VITE_API_BASE`,正式站 secret `BACKEND_API_BASE`)後,編發器可連接 Threads 並立即/排程發佈,由 `worker/`([`docs/BACKEND.md`](docs/BACKEND.md))的 cron 到點自動發佈;未設定=完整半自動模式;2026-09-04 已部署、2026-09-07 端到端驗收通過)。進行中:**IA 重整**([`docs/IA-PLAN.md`](docs/IA-PLAN.md),Phase 1 已完成)。接續開發候選:IA 重整 Phase 2–5、階段三 IG/X 代發、階段四 Web Push 提醒、遠期 F8。各階段細節與後端形態比較見 [`docs/HANDOFF.md`](docs/HANDOFF.md) 架構決策記錄。
