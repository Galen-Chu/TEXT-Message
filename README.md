...
| 編輯器 Text | 來源參考(郵件 / 社群歷史)、AI 語氣改寫與自訂指令、AI 郵件摘要、**AI 產生平台版本**(依勾選平台生成各版,可編輯後附加或存為範本)與**建議標籤**、文庫插入(含 `{{變數}}` 填值)、社群媒體挑選、多平台預覽與字數上限檢查;**YouTube 影片/Shorts 直接上傳**(草稿作為標題與說明,支援原生預約發佈);草稿自動保存於瀏覽器,可捨棄重來 |
...
**前端為主、後端輔助(可選)**。已完成:排程管理與發佈輔助(階段一)、**YouTube 影片/Shorts 上傳**(階段二,瀏覽器端 Google OAuth、零後端)、**文管庫(現文庫)功能深化四期**(範本填值、平台變體、發文趨勢、AI 平台版本與標籤建議——[`docs/LIBRARY-PLAN.md`](docs/LIBRARY-PLAN.md))、**Threads 平台代發前端串接**(階段三——設定後端網址(`VITE_API_BASE`,正式站 secret `BACKEND_API_BASE`)後,草稿頁可連接 Threads 並立即/排程代發,由 `worker/`([`docs/BACKEND.md`](docs/BACKEND.md))的 cron 到點自動發佈;未設定=完整半自動模式;2026-09-04 已部署並完成 OAuth 端到端驗證)。進行中:**IA 重整**([`docs/IA-PLAN.md`](docs/IA-PLAN.md),Phase 1 已完成)。接續開發候選:IA 重整 Phase 2–5、階段三 IG/X 代發、階段四 Web Push 提醒、遠期 F8。各階段細節與後端形態比較見 [`docs/HANDOFF.md`](docs/HANDOFF.md) 架構決策記錄。
