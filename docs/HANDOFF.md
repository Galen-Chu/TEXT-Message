...
- **實作補充(2026-09-03,階段三前端串接)**:設定後端(`VITE_API_BASE`)且勾選 Threads 時,出現「🧵 Threads 代發(後端輔助)」卡片——新分頁完成 Meta 授權(`?threads=connected` 回跳偵測)後可立即代發(成功寫入發文歷史)或排程代發(同步雲端佇列+本地排程);定排程頁另有「Threads 雲端佇列」卡(狀態/取消)。未設定後端的建置完全不出現相關 UI。
...
- **Purpose**: 管理三大類可套用草稿的內容,以分頁(segmented tab)切分職責:
  ...
  - **草稿管理**:保存的編輯成果(`DraftDoc` 集合,依更新時間排序)——操作:開啟至編輯器 / 複製 / 刪除;無分類、無「+ 新增內容」(草稿由編輯器產生)。
...
- **草稿插入邏輯**: 草稿頁「從文庫插入」modal 會合併「訊息管理」與「文案管理」兩個資料集一併列出供選擇,不分頁籤。
