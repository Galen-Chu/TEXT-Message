...
## 導覽結構(側邊欄,由上到下)
1. 文管 Dashboard(原首頁總覽)
2. 郵件匣 Gmail(原 Gmail 郵件匣)
3. 自媒體 Social(原社群媒體,發文歷史)
4. 編發器 Text(原「草稿撰寫」,2026-09-07 前稱「編輯器 Text」)
5. 定排程 Task(原排程管理)
6. 文庫 Library(原文管庫:訊息管理 / 文案管理)

> 導覽順序 v3(2026-09-04 IA 重整,決策見 `docs/IA-PLAN.md` D1):編輯器在定排程之前;品牌字串為「TEXT-Message」。請勿沿用 v1/v2 排列。

## Screens / Views

### 1. 文管 Dashboard(原首頁總覽)
- **Purpose**: 每日進入的總覽頁,快速看到本週統計、近期排程、Gmail 靈感。
- **Layout**: 左側固定 232px 側邊欄 + 右側主內容區(padding 32px 40px)。頂部標題列(問候語 + 日期 + CTA「瀏覽文庫」;原「從 Gmail 建立草稿」已於 2026-09-04 移除,見 `docs/IA-PLAN.md` D7)。下方 4 欄統計卡片橫排。再下方左右各半的「近期排程」與「來自 Gmail 的靈感」卡片。
...
- **Note**: 頁面標題已改為「郵件匣 Gmail」(沿革:Gmail 收件匣 → Gmail 郵件匣 → 郵件匣 Gmail,2026-09-04 IA 重整),實作時請統一沿用此名稱(Draft 頁空狀態按鈕亦已統一為「前往郵件匣挑選」)。實作補充:已連線時支援「載入更多」分頁(Gmail pageToken)與視窗重回前景靜默刷新(60 秒冷卻)。**2026-09-03 唯讀輔助**:已連線且帳號有使用者標籤時,分類 chip 下方出現「Gmail 標籤」第二篩選列(點擊過濾 labelIds;僅連線時生效);每封信新增「自動分類」按鈕——以預填 `from:<寄件者>` 的搜尋開啟 Gmail,由使用者建立原生篩選器(`services/gmail/filterLink.ts`)。
...
- **實作補充(2026-09-02)**:「標記已發佈」產生的真實記錄寫入 `publishedHistory`(隨其他使用者內容持久化);有真實記錄時本頁完全取代示範資料(同 Gmail 連線的分流哲學),「(示範資料)」提示僅在無真實記錄時顯示。文管庫深化第三期:頁首新增發文趨勢完整版(30/90 天切換、各平台計數、時段分佈、連續/最長天數、發文日形態;`TrendsPanel`,僅計真實記錄、門檻 5 筆,未達顯示累積提示);Dashboard 另有精簡版(近 30 天平台計數+連續天數)。
- **實作補充(2026-09-07,IA Phase 5 一期)**:趨勢面板下方新增「📬 最新互動」卡——顯示最近 5 封分類為「互動通知」的信件(平台留言/提及/追蹤通知,唯讀;真實信件可「查看信件」以 Gmail 搜尋開啟,回覆一律回平台 App)。評估與二期方案見 `docs/NOTIFY-PLAN.md`。

### 4. 編發器 Text(原「草稿撰寫」,2026-09-07 前稱「編輯器 Text」)
- **Purpose**: 核心編輯頁——來源參考(郵件或社群歷史貼文)+ 文字編輯 + AI 語氣輔助 + 文庫插入 + 社群媒體挑選 + 多平台預覽與字數檢查。
- **Layout**: 空狀態時置中提示卡,提供三個入口:「前往郵件匣挑選」「從社群媒體挑選」「空白草稿開始撰寫」。有目標時三段式:左側 280px 原始郵件參考卡(僅來源為 Gmail 時顯示)+ 右側主欄(工具列含「📋 從文庫插入」與「📣 從社群媒體挑選」兩個按鈕、AI 語氣 chip 列、textarea 編輯框、文檔類型卡——類型 chips+「捨棄草稿」「儲存草稿」按鈕(2026-09-08 移入本卡)、平台勾選 chip、逐平台預覽卡)。草稿內容、平台選擇與來源自動持久化於 localStorage(重新整理不消失),「捨棄草稿」清空回到空狀態。**「加入排程」按鈕已於 2026-09-08 移除**(與 Threads 排程發佈重複;`store.confirmSchedule` 保留為內部能力)。
...
- **實作補充(2026-09-03,階段三前端串接;2026-09-07 D12 用語更新)**:設定後端(`VITE_API_BASE`)且勾選 Threads 時,出現「🧵 Threads 發佈(後端輔助)」卡片——新分頁完成 Meta 授權(popup 返回模式:授權分頁以 postMessage 回報原分頁並自動關閉,原分頁收到訊息或重回前景即重查連線)後可立即發佈(成功寫入發文歷史)或排程發佈(同步雲端佇列+本地排程);定排程頁另有「Threads 雲端佇列」卡(狀態/取消)。未設定後端的建置完全不出現相關 UI。
- **實作補充(2026-09-02,文管庫深化第四期)**:工具列新增「✨ 產生平台版本」(Gemini 依勾選平台生成各版 → 可編輯面板 → 附加到草稿(D3 `[平台名 版]` 格式)或存為文案範本)與「#️⃣ 建議標籤」(3–5 個可點擊 chips,單個/全部加入草稿尾端);皆 BYOK——無 key 顯示按鈕但點擊僅提示(D5),失敗草稿不動、僅 toast;服務層見 `services/gemini/variants.ts`。
- **實作補充(2026-09-07,IA Phase 3)**:平台卡上方新增「文檔類型」chips(草稿/文案/訊息,預設文案)——影響 AI 語氣改寫與自訂指令的生成文體(draft=電子報長文、copy=社群貼文、message=留言回覆),並決定「儲存草稿」寫入文庫草稿管理的 kind 歸類;轉郵件/空白開始/捨棄重置為文案,開啟文庫草稿則跟隨該文檔 kind。決策見 `docs/IA-PLAN.md` D9。
...
- **實作補充(2026-09-07,IA Phase 4)**:排程對應三大類——`docKind` 可選欄位(舊資料顯示/篩選視為文案),建立三路徑(編輯器加入排程帶當前文檔類型、手動新增 modal 可選、Threads 排程代發跟隨);本頁新增類別篩選 chips(全部/草稿/文案/訊息,套用於選定日與所有排程兩卡)與排程列類別徽章;週曆與逾期橫幅不受篩選影響。決策見 `docs/IA-PLAN.md` D10。

### 6. 文庫 Library(原文管庫,原「罐頭訊息庫」重構)
- **Purpose**: 管理三大類可套用草稿的內容,以分頁(segmented tab)切分職責:
  - **訊息管理**:通用文字片段,適合直接套用(節慶祝賀、業配/產品促銷、粉絲互動、常見問答、感謝訊息)。
  - **文案管理**:完整社群貼文草稿範本,依內容企劃分類(日常分享、新品/業配、活動宣傳、品牌故事)。
  - **草稿管理**:保存的編輯成果(`DraftDoc` 集合,依更新時間排序;**全新使用者內建 3 組預設電子郵件範本**——感謝來信回覆/婉拒合作/電子報骨架,已有紀錄或清空過則不重複種入)——操作:開啟至編輯器 / 複製 / 刪除;無分類、無「+ 新增內容」(草稿由編發器產生)。
- **Layout**: 頁首標題「文庫 Library」+「+ 新增內容」按鈕(草稿管理分頁隱藏)。標題下方為三個分頁按鈕(訊息管理 / 文案管理 / 草稿管理),訊息與文案切換後下方分別呈現各自的左側分類清單(180px)+ 右側搜尋框 + 2 欄卡片網格;草稿管理為說明行 + 卡片網格(無分類/搜尋)。
- **新增內容邏輯**: 「+ 新增內容」開啟同一個 modal(標題「新增內容」),依目前所在分頁決定寫入「訊息管理」或「文案管理」的資料集,並使用該分頁當前選取的分類作為新項目分類。
- **編輯/刪除邏輯**: 每張內容卡提供「編輯」(開啟同一個 modal,預填標題與內容,儲存僅更新標題/內容、分類維持原值)與「刪除」(即時移除,toast 回覆)按鈕,與既有「套用編發器」「複製」並列。
- **草稿插入邏輯**: 編發器「從文庫插入」modal 會合併「訊息管理」與「文案管理」兩個資料集一併列出供選擇,不分頁籤。
- **實作補充(2026-09-07,IA Phase 2 三大類文檔)**:新增 `DraftDoc { id, kind: 'draft'|'copy'|'message', title, text, platforms, sourceId, updatedAt }` 集合(`drafts[]` + `activeDraftId`)持久化於 `text-message:v2` **加值欄位**——舊版單一草稿緩衝(draftText/draftPlatforms/draftSourceId)照舊驅動編輯器,首次載入自動遷移一筆 `kind='draft'` 文檔。編輯器「儲存草稿」由佔位 toast 升級為真實動作:追蹤中(activeDraftId)原地更新,否則建立新文檔(標題取內容前 12 字);轉郵件/空白開始/捨棄皆重置追蹤。決策細節見 `docs/IA-PLAN.md` D8。
- **Components**: 分頁按鈕(選中態 `#6C5CE7` 底白字);分類清單項(選中態 `#F1EDFF` 底 / `#6C5CE7` 字);內容卡片(分類 pill + 標題 + 內文 + 「套用編發器」主按鈕 + 「複製」次按鈕)。
...
5. **2026-09-02 收尾歸檔**:社群串接開發暫停(維護者時間因素)——階段 0/1/2 完成、階段 3 暫停於第一增量(`worker/` 骨架歸檔保留)、階段 4 未開工;**文管庫功能深化提前為現行工作流**,規劃設計見 `docs/LIBRARY-PLAN.md`(四期已於同日全數完成)。
6. **2026-09-03 郵件管理定位決議(C+B)**:Gmail 維持 `gmail.readonly` 紅線——不申請 `gmail.modify`、不自動貼 Gmail 標籤(打破 SETUP §9 的唯讀承諾,且純前端只在網站開著時能動,比 Gmail 原生篩選器差)。例行郵件(電子報/帳單)自動歸檔 = Gmail 原生篩選器的職責(伺服器端 24/7)。app 採「單純化為體(C):郵件=內容來源」+「唯讀輔助(B)」:使用者 Gmail 標籤作為收件匣第二篩選維度、每封信「自動分類」深連結(預填 from: 搜尋開 Gmail,使用者自行建立篩選器)。信任分層原則:app 內分類=建議層(可全自動),任何影響真實信箱的動作=行動層(必經確認)。
...
3. **階段 3(後端輔助)**:平台代發,順序 Threads(免費、審核較輕)→ IG(需商業帳號+綁粉專)→ X(量計費約 $0.015/則);cron 僅為無原生排程的平台存在;LINE 永遠手動。第一增量(worker 骨架)已歸檔;**前端串接於 2026-09-03 完成**(`services/backend` + `useThreadsProxy` + 代發 UI;worker 加 `/api/threads/status`;正式站以 secret `BACKEND_API_BASE` 啟用),待後端部署後端到端實測
