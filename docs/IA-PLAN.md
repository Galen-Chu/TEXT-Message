...
- **D13(UX 優化二批,2026-09-08 回歸驗收回饋)**:①Dashboard 標題「早安,小日」改「文管 Dashboard」;②側欄左下改產品識別塊(「文」logo+文管庫+「社群媒體每日發文工具」);③Gemini 429/5xx 不再直接失敗——自動換下一個候選模型(免費方案各模型限額獨立)+800ms 換檔等待+console 診斷紀錄,錯誤文案說明免費額度限制;④非同步操作防呆:Threads 發佈/排程加同步鎖(busyRef,連點回 null 靜默忽略)+按鈕進行中樣式(半透明/not-allowed/「發佈中…」),AI 按鈕群原有同款樣式;⑤編發器「捨棄/儲存草稿」按鈕移入文檔類型卡,「加入排程」按鈕移除(與 Threads 排程發佈重複,`confirmSchedule` 保留為內部能力)。
- **D12(UX 優化,2026-09-07 晚,Threads 首次全產品路徑實測回饋)**:①文庫「套用到草稿」按鈕改「**套用編發器**」(與頁籤同名;變數填值 modal 文案同步);②「代發」用語全面改「**發佈**」(Threads 發佈/立即發佈/排程發佈/已發佈至 Threads);③「編輯器 Text」更名「**編發器 Text**」(涵蓋編輯+發佈的內容核心);④草稿管理內建**預設電子郵件範本 3 組**(感謝來信回覆/婉拒合作/電子報骨架;僅全新使用者種入——已有 drafts 紀錄或曾清空則尊重現況,不重複種入);⑤OAuth 授權分頁改 **popup 返回模式**:connect 不再 `noopener`,授權分頁完成後以 `postMessage` 回報原分頁並自動關閉,原分頁收到訊息或視窗重回前景即重查連線(備援:無 opener 時維持原通知分頁行為;訊息接收端檢查同源)。
