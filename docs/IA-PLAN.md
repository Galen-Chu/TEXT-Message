...
- **D13(UX 優化二批,2026-09-08 回歸驗收回饋)**:①Dashboard 標題「早安,小日」改「文管 Dashboard」;②側欄左下改產品識別塊(「文」logo+文管庫+「社群媒體每日發文工具」);③Gemini 429/5xx 不再直接失敗——自動換下一個候選���型(免費方案各模型限額獨立)+800ms 換檔等待+console 診斷紀錄,錯誤文案說明免費額度限制;④非同步操作防呆:Threads 發佈/排程加同步鎖(busyRef,連點回 null 靜默忽略)+按鈕進行中樣式(半透明/not-allowed/「發佈中…」),AI 按鈕群原有同款樣式;⑤編發器「捨棄/儲存草稿」按鈕移入文檔類型卡,「加入排程」按鈕移除(與 Threads 排程發佈重複,`confirmSchedule` 保留為內部能力)。
