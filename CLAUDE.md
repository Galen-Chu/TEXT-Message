...
3. **IA 重整(2026-09-04 啟動,現行工作流)** — 決策記錄與分期見 `docs/IA-PLAN.md`。**Phase 1–3 已完成**(2026-09-07):Phase 1 六頁籤命名 v3 與品牌 TEXT-Message;Phase 2 三大類文檔模型(`DraftDoc` 集合+遷移、文庫草稿管理分頁、「儲存草稿」真實化);Phase 3 Gemini 依文檔類型分流(編輯器文檔類型 chips+prompt 分流)。**剩餘:Phase 4(排程類別維度)→ Phase 5(Social 互動通知——先出評估文件,候選一期為 Gmail 通知信分類)**
