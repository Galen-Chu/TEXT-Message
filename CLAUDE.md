...
**2026-09-09:Drive 首次真實串接+搜尋 q 語法 bug 修復、全站配色 WCAG AA 修正、技術債評估定案。**維護者另建 Cloud 專案與獨立 OAuth client 啟用 Drive API(本機 `.env.local` 已設 `VITE_DRIVE_CLIENT_ID`;正式站待 GitHub secret+`deploy.yml` 注入,`.env.example` 已補說明);首次真打 Drive API 揭漏 `buildListQuery` mimeType 子句語法錯誤(400 invalid q),已修+測試斷言補完整請求形狀(09-04 串接紀律重演案例)。配色:深色模式 `color-scheme` 缺失致輸入框白底白字(對比 1.19:1)等多項未達 AA,全數修正;brand/accent 分離「文字色調/填充 fill」雙 token(深色模式文字自動轉亮);btn-accent 白字→深棕字;輸入框補 focus 視覺;稽核腳本 `scripts/contrast-audit.mjs`(PlatformBadge 平台識別色除外,識別優先)。技術債遷移順序定案:**React 19 → Vite 8+Vitest 4+plugin-react v6 綁同一步**(Vitest 4 要求 Vite ≥6,唯一硬耦合)**→ TS 7**(`tsc -b` 已支援;`moduleResolution: bundler` 無虞)。**09-10 進行中:FB 粉專發佈串接**(粉專建立中;工程側:單步 Pages API、worker `threads/` 模組鏡像、FB 原生排程參數)。同日完成:正式站 Drive 注入的 `deploy.yml` 段(secret `DRIVE_CLIENT_ID`,缺漏不失敗;**待維護者加 GitHub secret 並 push 後生效**)、`SETUP.md` 補 Drive 章節(§12)與 Cloudflare token 取得/刪除流程(文件最前段,含「用完即刪、不貼對話」紀律)。待辦:GitHub secret `DRIVE_CLIENT_ID` 設定+部署驗證、Cloudflare token 儀表板刪除(維護者手動)、Google API 稽核申請。
